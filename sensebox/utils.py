...
# This function works but takes a lot of time for data fetching.
...
# Update or create doesn't get all the data. Some datas is missing while we use this approach. Mostly becoz of inconsistent timestamps, concurrency issues
# from asgiref.sync import sync_to_async
...
# async def fetch_and_store_datas(city):
    # base_path = '/app/tracks' if os.path.exists('/app') else './tracks'
...
#             timestamp = make_aware(datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%fZ"))
...
#             # Ensure timestamp consistency for created/updated dates
#             created_at = make_aware(datetime.strptime(created_at, "%Y-%m-%dT%H:%M:%S.%fZ"))
#             updated_at = make_aware(datetime.strptime(updated_at, "%Y-%m-%dT%H:%M:%S.%fZ"))
...
#             )
#             if created:
#                 print(f"Created new box: {box_id}")
#             else:
...
#             # Track table debug
#             existing_track = await sync_to_async(TracksTable.objects.filter(box_id=box, timestamp=timestamp).first)()
#             if existing_track:
#                 print(f"Updating track data for box ID {id}, Timestamp {timestamp}")
#                 existing_track.tracks = track
#                 await sync_to_async(existing_track.save)()
#             else:
#                 print(f"Creating new track data for box ID {id}, Timestamp {timestamp}")
#                 await sync_to_async(TracksTable.objects.create)(
#                     box_id=box,
#                     timestamp=timestamp,
#                     tracks=track,
#                     city=city,
...
#             # Check for duplicates in TracksTable
#             duplicate_tracks = await sync_to_async(TracksTable.objects.values('box_id', 'timestamp')
#                 .annotate(count=Count('id')).filter(count__gt=1))()
#             if duplicate_tracks:
#                 print(f"Warning: Duplicate track records found: {duplicate_tracks}")

#         await asyncio.gather(*(fetch_measurements(entry) for entry in id_list))
#         print("Data successfully updated/created.")

#     return {"status": "Data collection successful. Check the admin page for the data."}
